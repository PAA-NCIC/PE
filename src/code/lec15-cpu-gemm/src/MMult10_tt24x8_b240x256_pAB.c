...
#define BT(i,j) b[(j)*ldb + (i)] //row major for packedB
...
void InnerKernel( int m, int n, int k, double *a, int lda,  
  ...
  for(j = 0; j < n; j+=nr) {
    if ( first_time )
      ...
      AddDot24x8(k, &packedA[i * k], mr, &packedB[ j*k ], k, &C(i,j), ldc);
...
void PackMatrixB( int k, double *b, int ldb, double *b_to ){
  ...
  for(j = 0; j < nr; j++) {
    b_column = &B(0, j);
    for(p = 0; p < k; p+=8) {
      tmp1 = _mm512_load_pd(b_column + p);
      _mm512_store_pd(b_to + p, tmp1);
      //*(b_to + p) = *(b_column+p);
    }
    b_to += k;
  }
