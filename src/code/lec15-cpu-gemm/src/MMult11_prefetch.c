...
static void InnerKernelv2( int m, int n, int k, double *a, int lda,  
                                       double *b, int ldb, 
                                       double *c, int ldc);
static void PackMatrixA( int, double *, int, double * );
static void PackMatrixB( int, double *, int, double * );

static void PackMatrixBv2( int, int, double *, int, double * );
static void PackMatrixAv2( int, int, double *, int, double * );
...
void MY_MMult( int m, int n, int k, double *a, int lda, 
                                    ...
                                    double *c, int ldc)
{
  double *packedA = (double *)aligned_alloc(4096, kc * mc *sizeof(double));
  double *packedB = (double *)aligned_alloc(4096, kc * nc *sizeof(double));
  int i, j, p, ib, jb, pb;
  for(p = 0; p < k; p += kc){
    pb = min(k-p, kc);
    for(j = 0; j < n; j += nc){
      jb = min(n-j, nc);
      PackMatrixBv2(pb, jb, &B(p, j), ldb, packedB);
      for (i = 0; i < m; i += mc){       
        ib = min(m-i, mc);
        PackMatrixAv2(ib, pb, &A(i, p), lda, packedA);
        InnerKernelv2(ib, jb, pb, packedA, ib, packedB, ldb, &C(i, j), ldc);
...
void InnerKernelv2( int m, int n, int k, double *packedA, int lda,  
                    double *packedB, int ldb, double *c, int ldc) {
  int i, j;
  for(j = 0; j < n; j+=nr) {
	  for(i = 0; i < m; i+=mr) {
      AddDot24x8(k, &packedA[i * k], mr, &packedB[ j*k ], nr, &C(i,j), ldc);
...
//#define PREFETCH_C(i,j) __builtin_prefetch ((char *)&C( (i)*8, j ), 0)
...
void PackMatrixAv2( int m, int k, double *a, int lda, double *a_to) {
  int i, j;
  for(i = 0; i < m; i+= mr) { 
    for( j=0; j<k; j++){ 
      double *a_ptr = &A( i, j );
      __m512d tmp1 = _mm512_loadu_pd(a_ptr);
      __m512d tmp2 = _mm512_loadu_pd(a_ptr+8);
      __m512d tmp3 = _mm512_loadu_pd(a_ptr+16);
      _mm512_storeu_pd(a_to, tmp1);
      _mm512_storeu_pd(a_to+8, tmp2);
      _mm512_storeu_pd(a_to+16, tmp3);
      a_to += mr;
    }
  }
}
...
void PackMatrixBv2( int k, int n, double *b, int ldb, double *b_to ){
