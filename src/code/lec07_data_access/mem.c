...
int main(){
    ...
    uint64_t sample_points = env_get_int("SAMPLE_POINTS", 4);         
    uint64_t repeat_count = env_get_int("REPEAT_COUNT", 100); 
    uint64_t access_region_start = env_get_int("ACCESS_REGION_START", 64);  // bytes >= 256
    uint64_t access_region_end = env_get_int("ACCESS_REGION_END", 1073741824);  // bytes
    uint64_t prefetch_count = env_get_int("PREFETCH_COUNT", 0);  // bytes
    ...
    for(uint64_t access_region_pow_2 = access_region_start; access_region_pow_2 < access_region_end; access_region_pow_2 *= 2){
        ...
        for(uint64_t access_region = access_region_pow_2; access_region < min(access_region_pow_2 * 2, access_region_end); access_region += step){
            test_memory_latency_multichain(access_region, access_count, repeat_count, prefetch_count,chains, cycle_file);
        }
    }

    test_memory_latency_multichain(access_region_end, access_count, repeat_count, prefetch_count, chains, cycle_file);
